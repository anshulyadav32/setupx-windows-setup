...
function Install-SetupX {
    ...
    try {
        ...
        Write-ColorOutput "  SUCCESS: Execution policy set to RemoteSigned for CurrentUser." "Green"
    } catch {
        Write-ColorOutput "  WARNING: Could not set execution policy. Current policy: $(Get-ExecutionPolicy)" "Yellow"
    }
    Write-ColorOutput ""
    ...
    if (-not (Get-Command scoop -ErrorAction SilentlyContinue)) {
        if ($isAdmin) {
            Write-ColorOutput "  WARNING: Running as Administrator - Scoop installation may be restricted." "Yellow"
            Write-ColorOutput "  TIP: For best results, run this script as a regular user." "Cyan"
        ...
        try {
            ...
            if ($isAdmin) {
                Write-ColorOutput "  TIP: Try running as a regular user for Scoop installation." "Cyan"
            }
        }
    } else {
